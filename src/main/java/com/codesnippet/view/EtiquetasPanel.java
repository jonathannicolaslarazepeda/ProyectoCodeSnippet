 ...
 * Aqui tengo la interfaz para crear, ver y eliminar etiquetas.
 * Es uno de los paneles mas simples porque las etiquetas solo tienen un nombre.
 */
public class EtiquetasPanel extends JPanel {

    // Instancio el controlador para manejar la logica de negocio de las etiquetas
    private EtiquetaController controller = new EtiquetaController();

    // Componentes de la interfaz grafica
    private JTable tabla;                      // Tabla donde se muestran las etiquetas
    private DefaultTableModel modelo;          // Modelo de datos de la tabla
    private JTextField txtNombre;              // Campo de texto para escribir el nombre de la etiqueta
    private Long idSeleccionado;               // Guardo el ID de la etiqueta seleccionada (para editar o eliminar)
    ...
    // Metodo para crear el formulario de arriba con el campo de nombre y los botones
    private void crearFormulario() {
        // Creo un panel con GridLayout de 3 filas y 2 columnas (suficiente para nombre y 2 botones)
        JPanel panel = new JPanel(new GridLayout(3, 2, 8, 8));
        panel.setBackground(AppTheme.PANEL);
        panel.setBorder(BorderFactory.createTitledBorder("Datos de etiquetas"));

        // Creo el campo de texto y le aplico el estilo del tema
        txtNombre = new JTextField();
        AppTheme.campo(txtNombre);
    ...
    private void crearTabla() {
        // Creo el modelo de la tabla con las columnas "ID" y "Nombre"
        ...
        // Pongo el boton de eliminar en un panel separado para centrarlo o alinearlo abajo
        JPanel p = new JPanel();
        p.setBackground(AppTheme.FONDO);
        p.add(btnEliminar);

        // Agrego la tabla en el centro (con scroll por si hay muchas etiquetas)
        add(new JScrollPane(tabla), BorderLayout.CENTER);
        // Agrego el panel del boton eliminar en la parte inferior (SOUTH)
        add(p, BorderLayout.SOUTH);
    }

    // Metodo para guardar una etiqueta nueva o actualizar una existente
    private void guardar() {
        try {
            // Creo el objeto Etiqueta con el texto del campo
            Etiqueta obj = new Etiqueta(txtNombre.getText());
    ...
    // Metodo para eliminar la etiqueta seleccionada
    ...
    // Metodo para cargar todas las etiquetas de la base de datos en la tabla
    private void cargarTabla() {
        // Borro todas las filas que haya actualmente para no duplicar datos
        modelo.setRowCount(0);

        // Pido la lista completa de etiquetas al controlador
        List<Etiqueta> lista = controller.listar();

        // Recorro la lista y agrego cada etiqueta como una fila nueva en la tabla
        for (Etiqueta o : lista) {
            modelo.addRow(new Object[]{o.getId(), o.getNombre()});
        }
    }
    ...
    // Metodo para dejar el formulario limpio y listo para crear una etiqueta nueva
